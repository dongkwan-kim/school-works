...
for scene=scenes
    ...
    for imd = 1:1
        ...
        for iwsz = 1:80
            ...
            for ifsz = 1:80
                fsz = ifsz;
                [depthMap, disparityMap] = estimateDepth(leftRectified, rightRectified, stereoParams, md, wsz, fsz);
                depthDiff = leftGtDepthRectified(:, 140:end-140) - depthMap(:, 140:end-140);
                
                mean_diff = mean(abs(depthDiff(:)));
                fprintf('%.2f (%.2f, %.2f, %.2f)\n', mean_diff, md, wsz, fsz);
                if minDiff > mean_diff
                    minDiff = mean_diff;
                    fprintf('\nMinimum Depth mean difference: %.2f', mean_diff);
                    fprintf('   md: %.2f, wsz: %.2f, fsz: %.2f \n', md, wsz, fsz);
                    fprintf('\n');
                end
            end
        end
    end
    fprintf('Min depth mean difference: %.2f', minDiff);
end
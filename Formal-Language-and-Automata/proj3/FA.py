...
def alphabet_2_string(alphabet, length):
    ...
    for i in range(length):
        for s in ref_string:
            for a in alphabet:
                tmp_string.append(str(s) + str(a))
        ref_string = [w for w in tmp_string]
        string = string + [w for w in tmp_string]
        tmp_string.clear()
    return  string
...
class NFA:
    ...
    def NFA_2_mDFA(self):
        ...
        for (a, b) in relation_state:
            path_from_a = {}
            path_from_b = {}
            temp_DFA.get_all_path_from_state(a, "", path_from_a)
            temp_DFA.get_all_path_from_state(b, "", path_from_b)
            for f_state in temp_DFA.final_state:
                if f_state in path_from_a.keys():
                    path_a2f = path_from_a[f_state]
                    b_dest = temp_DFA.get_dest_of_state(b, path_a2f)
                    if b_dest not in temp_DFA.final_state:
                        filling_table[(a, b)] = False
                if f_state in path_from_b.keys():
                    path_b2f = path_from_b[f_state]
                    a_dest = temp_DFA.get_dest_of_state(a, path_b2f)
                    if a_dest not in temp_DFA.final_state:
                        filling_table[(a, b)] = False
        ...
        for (a, b) in relation_state:
            for p_inp in possble_input:
                ...
                if (a_dest in temp_DFA.final_state) ^ (b_dest in temp_DFA.final_state):
                    filling_table[(a, b)] = False
                    break
        for ((a, b), c) in filling_table.items():
...
class DFA:
    ...
    def get_all_path_from_state(self, state, bef_inp, marked_dic):
        if state in marked_dic.keys():
            return
        marked_dic[state] = bef_inp
        child = self.t_func.g_dic[state].items()
        for (inp, next) in child:
            if state == next:
                marked_dic[state] = inp
            self.get_all_path_from_state(next, bef_inp + inp, marked_dic)
    ...
    def summary(self, prefix="s"):
        new_states = dict([(self.state[i], prefix+str(i)) for i in range(len(self.state))])
        new_states["empty"] = "empty"
        new_t_func = TransitionFunctionDFA("")
        new_t_func.g_dic.clear()
        for (s1, inp_s2) in self.t_func.g_dic.items():
            for (inp, s2) in inp_s2.items():
                if new_states[s1] in new_t_func.g_dic.keys():
                    new_t_func.g_dic[new_states[s1]][inp] = new_states[s2]
                else:
                    new_t_func.g_dic[new_states[s1]] = {}
                    new_t_func.g_dic[new_states[s1]][inp] = new_states[s2]
        self.state = sorted(list(new_states.values()))
        self.t_func = new_t_func
        self.init_state = new_states[self.init_state]
        self.final_state = [new_states[s] for s in self.final_state]
        return self
